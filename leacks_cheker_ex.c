...
static void add_call(t_call *call) {
    ...
    if (position == 0) {
        fprintf(__stderrp, "There is no room left for `allocs_array`\n"
...
static void remove_call(long address) {
    ...
    fprintf(__stderrp, "Trying to free memory that have already freed or "
...
static void print_call(t_call *call) {
    fprintf(__stderrp, "Leak at address: 0x%lx | Leaked %ld bytes\n%s\n\n",
...
static void malloc_init(void) {
    ...
    if (real_malloc == 0)
        fprintf(__stderrp, "Error in `dlsym`: %s\n", dlerror());
