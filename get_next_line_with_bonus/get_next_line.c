...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	chr_read = save_buffer(fd, &buf, &stash[fd]);
	while (chr_read > 0)
	{
		if (get_next_line_0(&line, &stash[fd], &tmp, &buf) == 1)
			return (line);
		chr_read = save_buffer(fd, &buf, &stash[fd]);
	}
	if (get_next_line_1(&line, &stash[fd], &tmp, &buf) == 1)
		return (line);
	if (ft_strlen(stash[fd]) == 0 && stash[fd] != NULL)
	{
		free(stash[fd]);
		stash[fd] = NULL;
	}
